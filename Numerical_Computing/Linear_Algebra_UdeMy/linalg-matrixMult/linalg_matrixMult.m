...
%Fazer o produto atraves da visão em camada
A = randn(5,3)
B = randn(3,6)
...
toc
%São iguais a menos de erros de ponto flutuante
tic
D = A*B;
toc
sum(round(C(:)-D(:))    ==0)
...
%%
%Estudo em Cima das Matrizes de Rotação e dos Angulos 
...
for i=1:length(thetas)
    ...
    plot(get(gca,'xlim'),[0 0],'k--')
    plot([0 0],get(gca,'ylim'),'k--')
    txt1 = sprintf('Rotação Pura: Mag(%.2f)', norm(w1));
    txt2 = sprintf('Rotação Impura: Mag(%.2f)', norm(w2));
    legend({txt1; txt2})
    title('Rotações')
    hold off
end
