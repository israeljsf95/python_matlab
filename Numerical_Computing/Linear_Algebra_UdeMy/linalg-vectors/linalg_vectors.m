...
% 2) a==b==c
%Provando o caso em que é bom
(a'*(a'*a))' ==  ((a'*a)'*a)

%%Code Challenge - Israel Jesus Santos FIlho
A1 = rand(4,6);
A2 = rand(4,6);
dps = zeros(1,size(A1,2));
for i = 1:size(A1,2)
    dp(i) = A1(:,i)'*A2(:,i)
end
%%
...
%O sinal do produto Escalar é Invariante ao produto por
%um escalar
...
dp1 = v1'*v2
dp2 = V1'*V2
%Ou seja, o sinal do produto escalar não é invariável a 
%multiplicação

%%
%Quarto Code Challenge
%Normalizando Vetores para terem norma unitária
...
h = ezmesh(@(s,t)v(1)*s + w(1)*t, @(s,t)v(2)*s + w(2)*t, @(s,t)v(3)*s + w(3)*t, [-1 1 -1 1]*2);
set(h, 'facecolor','r', 'cdata', ones(40), 'EdgeColor', 'none')
plot3([0 v(1)],[0 v(2)],[0 v(3)],'k','linew',3)
plot3([0 w(1)],[0 w(2)],[0 w(3)],'g','linew',3)
plot3([0 v_n(1)],[0 v_n(2)],[0 v_n(3)],'b','linew',3)
plot3([0 w_n(1)],[0 w_n(2)],[0 w_n(3)],'y','linew',3)
axis square
v'*w
v_n'*w_n
%%
...
normvec = normvec/sqrt(normvec'*normvec)
[X,Y] = meshgrid(-4:4,-4:4);
z = -(normvec(1)*X + normvec(2)*Y)/normvec(3);
surf(X,Y,z)
shading interp
colormap('gray')
plot3([0 normvec(1)],[0 normvec(2)],[0 normvec(3)],'c','linew',3)
axis square
grid on, rotate3d on
